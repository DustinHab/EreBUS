...
- The log says what was fetched (`web: get http://... -> 200, 532 bytes, text/html, gzip`), what the styles did (`web: styles: 812 rules, 2 of 2 sheets, 14 parts hidden, 3 folds`), every fold opened or shut, every picture decoded, every cookie taken, and every bookmark kept.
...
### 18.6 A program the system starts itself: the picture decoder
...
The program is built by clang and lld on the host into the image format of 18.4 (`programs/decoder/program.ld` lays the code at `0x1000000` and the data at `0x1100000`; `tools/mkimage.py` writes the head in front of the two segments and wraps the image as a C array) and carried by the kernel as those bytes. In the self-build (15) the machine's own compiler and linker build the same program the same way (`tools/selfdecoder.sh`), so a self-built kernel's decoder is the machine's compiler's work; the on-machine build takes it as the text of its image, since the machine cannot yet turn a program it built into such a text itself.
