...
- The board's other processors are brought up from the ACPI MADT and the scheduler runs across all of them (SMP). This path is exercised in the tests under QEMU `-smp` (the boot smoke and three battery lanes); a recorded multi-core boot on this board is still to be added here.
