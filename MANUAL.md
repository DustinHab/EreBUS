...
For EreBUS 0.9.2. This manual is updated with every release; the version it describes is the one on the releases page.
...
- **Styles:** of a stylesheet, the part that changes what the page says is honoured: what a rule hides (`display: none`, `visibility: hidden`, the `hidden` attribute) stays hidden; `font-weight` sets words strong, `color` colours them (a colour too light for the page is left out), `text-align` centres a line or sets it right, `list-style: none` drops the bullet, `display: block` breaks a line where the markup would not and `display: inline` keeps one where it would. The page's `<style>` blocks and the sheets it links (`<link rel="stylesheet">`, fetched before the pictures) are read in the page's order, the `style` attribute over them, `!important` and specificity as the cascade says; media queries on width, height, `screen` and `print` and the usual preferences are evaluated for this screen. Selectors read: tags, `.class`, `#id`, `[attr]`, `[attr=value]`, descendant and child; one with a pseudo-class or -element never matches. Nothing is laid out in boxes -- there is one font and one column -- so a page that is its layout reads in its order. Up to eight sheets of 2 MiB together and 10800 rules per page.
