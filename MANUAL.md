...
- The image is loaded with its code at `0x1000000` (read and execute) and its data at `0x1100000` (read and write, the zeroed room after it), and entered at its entry point with a stack of 16 KiB whose top is `0x800000`. Nothing else is mapped: no kernel, no other program, no device.
...
An image begins `EBX2`, then four ints: the length of the code, the length of the data, the zeroed room after the data, and the entry's offset into the code; the code and the data follow. `EBX1`, without the entry, is still read and enters at the first byte of the code. The compiler and the linker make these (10); `tools/cchost` makes the same on the host. A compiled task sent to another machine (9.3) is C source, not an image: the far machine builds it itself, so no foreign binary runs.
