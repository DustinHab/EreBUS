...
bool crypto_selftest(void)
{
    ...
    /* SHA-512("abc") */
    {
        ...
        static const u8 want[64] = {
            ...
            0xa5,0x4c,0xa4,0x9f };
        sha512("abc", 3, out);
        if (!eq(out, want, 64)) { kprintf("crypto: sha512 vector failed\n"); return false; }
    }
    ...
    /* Ed25519, RFC 8032 section 7.1 test 1: the empty message. */
    {
        ...
        static const u8 want_sig[64] = {
            ...
            0x8e,0x7a,0x10,0x0b };
        u8 pk[32], sig[64];
        ed25519_public(pk, seed);
        if (!eq(pk, want_pk, 32)) { kprintf("crypto: ed25519 key vector failed\n"); return false; }
        ed25519_sign(sig, seed, pk, "", 0);
        if (!eq(sig, want_sig, 64)) { kprintf("crypto: ed25519 signature vector failed\n"); return false; }
        if (!ed25519_verify(pk, "", 0, sig)) { kprintf("crypto: ed25519 verify failed\n"); return false; }
        sig[0] ^= 1;
        if (ed25519_verify(pk, "", 0, sig)) { kprintf("crypto: ed25519 accepted a bent signature\n"); return false; }
    }
