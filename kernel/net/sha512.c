...
static inline u64 rotr(u64 x, u32 n) { return (x >> n) | (x << (64 - n)); }

static void block(sha512_ctx *c, const u8 *p)
{
    u64 w[80];
    for (u32 i = 0; i < 16; i++)
...
void sha512_update(sha512_ctx *c, const void *data, u64 len)
{
    ...
    while (len) {
        ...
        p += take;
        len -= take;
        if (c->fill == 128) { block(c, c->buf); c->fill = 0; }
    }
}
...
void sha512_final(sha512_ctx *c, u8 out[64])
{
    ...
    u8 len[16];
    for (u32 i = 0; i < 8; i++) len[i] = 0;
    for (u32 i = 0; i < 8; i++) len[8 + i] = (u8)(bits >> (56 - i * 8));
