...
| tools/webtest.sh | the browser against a server on the host: a page with utf-8 prose, links, a png and a jpeg decoded, a link followed by keyboard and back, a form posted with a cookie set and sent on the redirect, a bookmark kept, gzip and chunked pages; the next boot has the bookmark and the cookie |
...
| tools/fuzz/run.sh | fuzzing under the sanitizers: the language tools, the certificate checker, the page renderer, the wire (frames, the tcp and http client, the door with ssh, the air), the pipe's datagrams sealed and plain, the tls client, the picture decoders and inflate (`sh tools/fuzz/run.sh <seconds> [lang pki html net pipe tls img]`) |
...
- The browser reads no scripts and no stylesheets; a page that is nothing without them is nothing here. Progressive jpeg and interlaced png show as their alternative text (so do svg and webp); a page is 2 MiB at most, a picture 1600 x 1200; pictures are fetched anew with every page; one connection at a time, so a page with many pictures fills in one by one.
