...
## Layout
    ...
    kernel/
      ...
      gfx/           framebuffer, font, shell (desktop), html lens, the picture decoder's job (picture.c)
      ...
      include/eb/    headers
    programs/
      decoder/       the picture decoder (png, jpeg, webp): a ring-3 program linked on its own, carried by the kernel as bytes
...
What 1.0 needs beyond this list is named in Known limits: more boards, an outside review with fuzzers run for hours, a build of a tag by a third party with the same hash, and the remaining parsers of foreign input out of ring 0 -- the picture decoders are out; the page and stylesheet readers, the certificate checker and the compiler are not.
...
- [x] The picture decoders (png, baseline jpeg, webp, with inflate) as a program in ring 3, `programs/decoder`, written against `sdk/erebus.h`: the browser lays a picture's bytes into a fresh process, the program answers where its pixels lie, the kernel reads them back through the program's own page tables and ends it; a decoder that faults, hangs or refuses costs that one picture (`tools/decoder-fault.sh`). Built by clang and lld into the machine's own image format (`programs/decoder/program.ld`, `tools/mkimage.py`) and carried by the kernel as bytes; the self-build builds it with the machine's own compiler (`tools/selfdecoder.sh`)
- [x] Self-build: `tools/selfbuild.sh` (host, cchost), `tools/selfkernel.sh` (on the machine, through the door, under KVM: 99 objects in ~30 s, then a second generation); the decoder program built the same way and sent in as the text of its image
