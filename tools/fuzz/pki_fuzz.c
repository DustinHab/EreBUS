...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    case 3: {                                        /* an ecdsa signature under a fixed point, bytes as der */
        ...
        uint8_t h[32];
        sha256(in, len, h);
        p256_verify_der(pub, h, in, len);
        if (len >= 65) p256_point_ok(in);
        break;
    }
    case 4: {                                        /* rsa: the bytes as modulus (first half) and signature (second half) */
        ...
        uint8_t h[32];
        sha256(in, len, h);
        rsa_verify_pkcs1_sha256(in, half, e, 3, h, in + half, half);
        rsa_verify_pss_sha256(in, half, e, 3, h, in + half, half);
        break;
    }
