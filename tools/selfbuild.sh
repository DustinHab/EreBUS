...
if [ -n "$KERNEL" ]; then
    ...
    # The programs the kernel carries first: the picture decoder and the
    # page renderer, each built with the same tools and wrapped as a
    # text of bytes the kernel carries.
    sh tools/selfprograms.sh || exit 1
    ...
    CS="$CS $OUT/decoder_image.c $OUT/renderer_image.c"
